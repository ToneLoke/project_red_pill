...
class LiveController extends AppController {
  ...
  constructor(model,io, id) {
	...
	async connected(socket){
		try {
			const game = await this._model.findOne({_id: this.gameId}).populate('adminId').populate('questions')
	...
	async updateGame(update){
		const game = await this._model.findOneAndUpdate({_id: this.gameId}).populate('adminId').populate('questions')
	}
}
